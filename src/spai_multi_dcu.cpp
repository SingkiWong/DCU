...
void distributeColumns(MultiDCU_Context *ctx, const CSC_Matrix *hostA, int baseColStart, int baseColEnd, bool useNnzBalance) {
    ...
    int totalCols = baseColEnd - baseColStart;
    if (totalCols <= 0) {
        printf("错误: 总列数必须大于0\n");
        return;
    }
    ...
    if (useNnzBalance && hostA->mPtr && hostA->nonzeroes > 0) {
        ...
        int currentCol = 0;
        for (int i = 0; i < ctx->numDCUs; i++) {
            ctx->colStart[i] = baseColStart + currentCol;
            ...
            if (i == ctx->numDCUs - 1) {
                ...
                while (currentCol < totalCols) {
                    int globalCol = baseColStart + currentCol;
                    int colNnz = hostA->mPtr[globalCol + 1] - hostA->mPtr[globalCol];
            ...
            ctx->colEnd[i] = baseColStart + currentCol;
        ...
        for (int i = 0; i < ctx->numDCUs; i++) {
            ...
            ctx->colEnd[i] = baseColStart + currentCol + colsForThisDCU;
            currentCol += colsForThisDCU;
    ...
    // 验证分配
    if (ctx->colEnd[ctx->numDCUs - 1] != baseColEnd) {
        printf("\n警告: 列分配不完整！分配了 %d 列，期望 %d 列\n",
               ctx->colEnd[ctx->numDCUs - 1], baseColEnd);
    }
...
//==============================================================================
// MPI 跨进程结果聚合（将各 MPI 进程的局部矩阵聚合到 rank 0）
//==============================================================================

#ifdef USE_MPI
void aggregateResultsMPI(CSC_Matrix *localM, CSC_Matrix *globalM,
                         int mpiRank, int mpiSize, int totalCols) {
    printf("MPI 进程 %d: 开始跨进程结果聚合...\n", mpiRank);

    // 1. 收集各进程的列范围信息
    int *colStarts = NULL;
    int *colEnds = NULL;
    int *nonzerosCounts = NULL;

    if (mpiRank == 0) {
        colStarts = (int*)malloc(sizeof(int) * mpiSize);
        colEnds = (int*)malloc(sizeof(int) * mpiSize);
        nonzerosCounts = (int*)malloc(sizeof(int) * mpiSize);
    }

    int localColStart = (totalCols * mpiRank) / mpiSize;
    int localColEnd = (totalCols * (mpiRank + 1)) / mpiSize;
    int localNonzeros = localM->nonzeroes;

    MPI_Gather(&localColStart, 1, MPI_INT, colStarts, 1, MPI_INT, 0, MPI_COMM_WORLD);
    MPI_Gather(&localColEnd, 1, MPI_INT, colEnds, 1, MPI_INT, 0, MPI_COMM_WORLD);
    MPI_Gather(&localNonzeros, 1, MPI_INT, nonzerosCounts, 1, MPI_INT, 0, MPI_COMM_WORLD);

    // 2. 复制局部矩阵到主机（从 DCU 到主机内存）
    int localCols = localColEnd - localColStart;
    int *h_localPtr = (int*)malloc(sizeof(int) * (localCols + 1));
    int *h_localIndex = NULL;
    double *h_localData = NULL;
    ...
    if (localNonzeros > 0) {
        h_localIndex = (int*)malloc(sizeof(int) * localNonzeros);
        h_localData = (double*)malloc(sizeof(double) * localNonzeros);

        CHECK_HIP_ERROR(hipMemcpy(h_localIndex, localM->mIndex,
                                   localNonzeros * sizeof(int),
                                   hipMemcpyDeviceToHost));
        CHECK_HIP_ERROR(hipMemcpy(h_localData, localM->mData,
                                   localNonzeros * sizeof(double),
                                   hipMemcpyDeviceToHost));
    }
    ...
    if (mpiRank == 0) {
        ...
        for (int i = 0; i < mpiSize; i++) {
            totalNonzeros += nonzerosCounts[i];
        }

        printf("MPI Rank 0: 总非零元数 = %d\n", totalNonzeros);

        // 分配全局矩阵
        globalM->nCol = totalCols;
        globalM->nRow = totalCols;
        globalM->n = totalCols;
        globalM->nonzeroes = totalNonzeros;

        int *h_globalPtr = (int*)malloc(sizeof(int) * (totalCols + 1));
        int *h_globalIndex = (int*)malloc(sizeof(int) * totalNonzeros);
        double *h_globalData = (double*)malloc(sizeof(double) * totalNonzeros);

        h_globalPtr[0] = 0;
        int currentOffset = 0;

        // 处理 rank 0 的数据（已经在主机内存中）
        for (int i = 0; i <= localCols; i++) {
            h_globalPtr[localColStart + i] = h_localPtr[i] + currentOffset;
        }
        if (localNonzeros > 0) {
            memcpy(h_globalIndex, h_localIndex, localNonzeros * sizeof(int));
            memcpy(h_globalData, h_localData, localNonzeros * sizeof(double));
        }
        currentOffset += localNonzeros;

        // 接收其他 rank 的数据
        for (int rank = 1; rank < mpiSize; rank++) {
            int rankColStart = colStarts[rank];
            int rankColEnd = colEnds[rank];
            int rankCols = rankColEnd - rankColStart;
            int rankNonzeros = nonzerosCounts[rank];

            if (rankCols > 0) {
                int *h_rankPtr = (int*)malloc(sizeof(int) * (rankCols + 1));
                MPI_Recv(h_rankPtr, rankCols + 1, MPI_INT, rank, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

                // 合并列指针
                for (int i = 0; i <= rankCols; i++) {
                    h_globalPtr[rankColStart + i] = h_rankPtr[i] + currentOffset;
                }
                free(h_rankPtr);
            }

            if (rankNonzeros > 0) {
                int *h_rankIndex = (int*)malloc(sizeof(int) * rankNonzeros);
                double *h_rankData = (double*)malloc(sizeof(double) * rankNonzeros);

                MPI_Recv(h_rankIndex, rankNonzeros, MPI_INT, rank, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                MPI_Recv(h_rankData, rankNonzeros, MPI_DOUBLE, rank, 2, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

                memcpy(h_globalIndex + currentOffset, h_rankIndex, rankNonzeros * sizeof(int));
                memcpy(h_globalData + currentOffset, h_rankData, rankNonzeros * sizeof(double));

                free(h_rankIndex);
                free(h_rankData);
            }

            currentOffset += rankNonzeros;
        }
        ...
        // 复制到 DCU
        CHECK_HIP_ERROR(hipMalloc((void**)&globalM->mPtr, sizeof(int) * (totalCols + 1)));
        CHECK_HIP_ERROR(hipMalloc((void**)&globalM->mIndex, sizeof(int) * totalNonzeros));
        CHECK_HIP_ERROR(hipMalloc((void**)&globalM->mData, sizeof(double) * totalNonzeros));

        CHECK_HIP_ERROR(hipMemcpy(globalM->mPtr, h_globalPtr,
                                   (totalCols + 1) * sizeof(int),
                                   hipMemcpyHostToDevice));
        CHECK_HIP_ERROR(hipMemcpy(globalM->mIndex, h_globalIndex,
                                   totalNonzeros * sizeof(int),
                                   hipMemcpyHostToDevice));
        CHECK_HIP_ERROR(hipMemcpy(globalM->mData, h_globalData,
                                   totalNonzeros * sizeof(double),
                                   hipMemcpyHostToDevice));
        ...
        printf("MPI Rank 0: 全局矩阵聚合完成\n");
    } else {
        // 其他 rank: 发送数据到 rank 0
        MPI_Send(h_localPtr, localCols + 1, MPI_INT, 0, 0, MPI_COMM_WORLD);
        if (localNonzeros > 0) {
            MPI_Send(h_localIndex, localNonzeros, MPI_INT, 0, 1, MPI_COMM_WORLD);
            MPI_Send(h_localData, localNonzeros, MPI_DOUBLE, 0, 2, MPI_COMM_WORLD);
        }
        printf("MPI Rank %d: 数据发送完成\n", mpiRank);
    }

    // 清理局部数据
    free(h_localPtr);
    if (h_localIndex) free(h_localIndex);
    if (h_localData) free(h_localData);
}
#endif
...
int main(int argc, char **argv) {
    ...
    // MPI下按列范围切分任务
    int baseColStart = (CSC_A->nCol * mpiRank) / mpiSize;
    int baseColEnd = (CSC_A->nCol * (mpiRank + 1)) / mpiSize;
    printf("MPI Rank %d/%d: 列范围 [%d, %d)\n", mpiRank, mpiSize, baseColStart, baseColEnd);
    ...
    if (mpiSize > 1) {
        ...
        // 仅在 rank 0 上释放原来的局部全局矩阵并使用最终聚合的矩阵
        if (mpiRank == 0) {
            ...
            devCSC_M_global = devCSC_M_final;
        }
    }
#endif
    ...
    if (mpiRank == 0) {
        ...
        // 其他 rank 在 MPI 聚合后已经发送数据，只需释放结构体
        if (devCSC_M_global) {
            if (devCSC_M_global->mPtr) hipFree(devCSC_M_global->mPtr);
            if (devCSC_M_global->mIndex) hipFree(devCSC_M_global->mIndex);
            if (devCSC_M_global->mData) hipFree(devCSC_M_global->mData);
