...
float StaticSPAIv20(CSC_Matrix *devA, CSC_Matrix *devM) {
    ...
    int *dev_n1;
    cudaMalloc((void **) &dev_n1, sizeof(int) * devA->nCol);
    int *dev_tI;
    int ssize = n2max * 8;
    ssize = min(ssize, devA->nCol);

    cudaMalloc((void **) &dev_tI, sizeof(int) * devA->nCol * ssize);

    int *dev_atomic;
    cudaMalloc((void **) &dev_atomic, sizeof(int) * devA->nCol);
...
//	    int *dev_mTmpPtr, *dev_mTmpIndex;
        double *dev_mTmpData;
//
//	    cudaMalloc((void**)&dev_mTmpPtr, sizeof(int) * (devA->nCol + 1)) ;
//      cudaMalloc((void**)&dev_mTmpIndex, sizeof(int) * devA->nonzeroes ) ;
        cudaMalloc((void **) &dev_mTmpData, sizeof(double) * devA->nonzeroes);
        ...
        for (int iter = 0; iter < itertions; iter++) {
            ...
            //computeJ2_SSPAIv10<32><< <blocksPerGrid, threadsPerBlock>> >(devA->mPtr, devA->mIndex, eGrid, dev_J, dev_jPTR, n2max, sK);
...
//			  ComputeTildeE2_SSPAIv10<32><< <blocksPerGrid, threadsPerBlock>> >(dev_E, dev_I, dev_iPTR, n1max, eGrid, sK);
        ...
        cudaMalloc((void **) &devM->mIndex, sizeof(int) * devM->nonzeroes);
        cudaMalloc((void **) &devM->mData, sizeof(double) * devM->nonzeroes);
        cudaMalloc((void **) &devM->mPtr, sizeof(int) * (devM->nCol + 1));

        cudaMemcpy(devM->mIndex, devA->mIndex, sizeof(int) * devM->nonzeroes, cudaMemcpyDeviceToDevice);
        cudaMemcpy(devM->mPtr, devA->mPtr, sizeof(int) * (devM->nCol + 1), cudaMemcpyDeviceToDevice);
...
//      exit(0);
        ...
        cudaMalloc((void **) &dev_J, sizeof(int) * devA->nCol * n2max);
        cudaMalloc((void **) &dev_jPTR, sizeof(int) * devA->nCol);

        cudaMalloc((void **) &dev_I, sizeof(int) * devA->nCol * n1max);
        cudaMalloc((void **) &dev_iPTR, sizeof(int) * devA->nCol);

        cudaMalloc((void **) &dev_tildeA, sizeof(double) * devA->nCol * n1max * n2max);
        cudaMalloc((void **) &dev_R, sizeof(double) * devA->nCol * n2max * n2max);
        cudaMalloc((void **) &dev_X, sizeof(double) * devA->nCol * n2max);
        cudaMalloc((void **) &dev_E, sizeof(int) * devA->nCol);
...
int main(int argc, char **argv) {
    ...
    cudaMalloc((void **) &devCSC_A->mPtr, sizeof(int) * (CSC_A->nCol + 1));
    cudaMalloc((void **) &devCSC_A->mIndex, sizeof(int) * CSC_A->nonzeroes);
    cudaMalloc((void **) &devCSC_A->mData, sizeof(double) * CSC_A->nonzeroes);

    cudaMemcpyAsync(devCSC_A->mData, CSC_A->mData, CSC_A->nonzeroes * sizeof(double), cudaMemcpyHostToDevice, 0);
    cudaMemcpyAsync(devCSC_A->mIndex, CSC_A->mIndex, CSC_A->nonzeroes * sizeof(int), cudaMemcpyHostToDevice, 0);
    cudaMemcpyAsync(devCSC_A->mPtr, CSC_A->mPtr, (CSC_A->nCol + 1) * sizeof(int), cudaMemcpyHostToDevice, 0);
    ...
    cudaMalloc((void **) &devCSR_A->mPtr, sizeof(int) * (devCSR_A->nRow + 1));
    cudaMalloc((void **) &devCSR_A->mIndex, sizeof(int) * devCSR_A->nonzeroes);
    cudaMalloc((void **) &devCSR_A->mData, sizeof(double) * devCSR_A->nonzeroes);

    devCSR_M->n = devCSC_M->n;
    devCSR_M->nonzeroes = devCSC_M->nonzeroes;
    devCSR_M->nCol = devCSC_M->n;
    devCSR_M->nRow = devCSC_M->n;
    cudaMalloc((void **) &devCSR_M->mPtr, sizeof(int) * (devCSR_M->nRow + 1));
    cudaMalloc((void **) &devCSR_M->mIndex, sizeof(int) * devCSR_M->nonzeroes);
    cudaMalloc((void **) &devCSR_M->mData, sizeof(double) * devCSR_M->nonzeroes);
    ...
    cudaMalloc((void **) &dev_x, sizeof(double) * devCSR_A->n);
    cudaMalloc((void **) &dev_b, sizeof(double) * devCSR_A->n);

    cudaMemcpyAsync(dev_x, x, devCSR_A->n * sizeof(double), cudaMemcpyHostToDevice, 0);
    cudaMemcpyAsync(dev_b, b, devCSR_A->n * sizeof(double), cudaMemcpyHostToDevice, 0);

    //csr_lpbicgstab(CSR_A, CSR_M, b, x, TOL, MAX_ITER) ;
    //csr_pbicgstab(CSR_A, CSR_M, b, x, TOL, MAX_ITER) ;
    //csr_bicgstab(CSR_A, b, x, TOL, MAX_ITER) ;
    //csr_lpgmres(CSR_A, CSR_M, b, x, TOL, MAX_ITER, 20);
    //csr_rpgmres(CSR_A, CSR_M, b, x, TOL, MAX_ITER, 20);
    cublas2_pbicgstabv2(devCSR_A, devCSR_M, dev_b, dev_x, TOL, MAX_ITER);
    //cublas2_bicgstab(CSR_A, b, x, TOL, MAX_ITER) ;
    //cublas2_lpgmres(devCSR_A, devCSR_M, dev_b, dev_x, TOL, MAX_ITER) ;
    ...
    cudaFree(devCSC_A->mData);
    cudaFree(devCSC_A->mIndex);
    cudaFree(devCSC_A->mPtr);
    cudaFree(devCSC_M->mData);
    cudaFree(devCSC_M->mIndex);
    cudaFree(devCSC_M->mPtr);

    cudaFree(devCSR_A->mData);
    cudaFree(devCSR_A->mIndex);
    cudaFree(devCSR_A->mPtr);
    cudaFree(devCSR_M->mData);
    cudaFree(devCSR_M->mIndex);
    cudaFree(devCSR_M->mPtr);
