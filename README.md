...
如果你要验证“接口是否可用、参数是否映射正确”，推荐先做 **CLI 接口烟雾测试**：

```bash
# 先编译 MPI 版本
make -f Makefile.multi mpi

# 运行接口测试脚本（会校验 ALGO_REPORT）
./experiments/test_api_cli.sh ./spai_multi_dcu_mpi matrices/circuit_2.mtx 1
```

脚本会自动验证：
- `--algo static` 默认映射到 `balanced_columns`
- `--algo dynamic` 默认映射到 `balanced_nnz`
- 手动 `--partition` 覆盖是否生效
- `--precondition-only` 下 `iteration_count=0`
